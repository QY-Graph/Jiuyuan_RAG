...
@app.route("/augment_by_file/user/<string:userid>",methods=["POST"])
def augment_by_file(userid):
    func_start_time = time.time()
    #Get query embedding
    EMBEDDING_URL="http://localhost:8846/embedding_bge_m3"
    EMBEDDING_HEADERS=HEADERS = {"Content-Type": "application/json", "Accept": "application/json"}
    ...
    query = ""
    file_list = []
    if ('message' in request_data) and ('file_list' in request_data):
        query = request_data['message']
    ...
    query_embedding = json.loads(r.content)
    #filter_ = [("userid",userid),("dialogueid",dialogueid)] 
    #filters_ = [MetadataFilter(key="docid",operator=FilterOperator.EQ,value = file_id) for file_id in file_list  ]
    file_filter = MetadataFilter(key="docid",operator=FilterOperator.IN, value = file_list)
    user_filter = MetadataFilter(key="userid",operator=FilterOperator.EQ,value = userid)
    filters = MetadataFilters(
        condition=FilterCondition.AND,
        filters= [user_filter,file_filter]
    )
    vector_store_query = VectorStoreQuery(
        query_embedding=query_embedding,
        similarity_top_k=20,
    ...
    #vector_store = JiuyuanVectorStore(schema_name="public",table_name="test1035", embed_dim=1024,\
    #    host='172.22.162.213', port='7474', user='default_user', password='', database_name='default_db')
    vector_store = JiuyuanVectorStore(schema_name="public",table_name=userid, embed_dim=1024,\
        host='172.22.162.11', port='7474', user='default_user', password='', database_name='default_db')
    #vector_store = JiuyuanVectorStore(chroma_collection=db.get_or_create_collection(f"{collection_id}"))
    #try:
    query_start = time.time()
    ...
    response_pairs = [(res, i) for i, res in enumerate(response) if res > 0]
    TOPK = 10
    sorted_pairs = sorted(response_pairs,key = lambda x : x[0],reverse=True)[0:min(TOPK,len(refs))]
    
    #Use top-k directly, test for no rerank
    #sorted_pairs = refs[0:5]
    #Make prompt
    chunk_list = [ {refs[pair[1]]:pair[0]} for i,pair in enumerate(sorted_pairs) ]
    if len(sorted_pairs)>0:
      ...
      status_ = 201
    response = app.response_class(
        json.dumps({"message":prompt,"text_chunks":chunk_list}),
        status=status_,
        mimetype='application/json'
    )
    func_end_time = time.time()
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=1237, debug=True, use_reloader=False)
